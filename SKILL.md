...
- Use `/tana <content>` as the primary trigger format in Codex command palette.
- If `/tana` is unavailable in the current thread, use `$tana <content>` as explicit fallback.
...
- Parse `/tana <content>` (or `$tana <content>`) and write the `<content>` body to Tana.
